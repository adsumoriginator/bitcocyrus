 ...
 */
 class Trade_model extends CI_Model {
	...
	function get_total_buy($pair_id=""){

		$where = array('status'=>'active','pair'=>$pair_id,'Type'=>'buy');
		$change_price = $this->db->select('SUM(Amount) AS sum_price')->where($where)->get('coin_order')->row();	
		
		if($change_price->sum_price==""){
		
			return 0;
		}else{


		return $change_price->sum_price;
		}

	}


		function get_total_sell($pair_id=""){

		$where = array('status'=>'active','pair'=>$pair_id,'Type'=>'sell');
		$change_price = $this->db->select('SUM(Amount) AS sum_price')->where($where)->get('coin_order')->row();	
			if($change_price->sum_price==""){
	...
	function forLowHighchart_pair($source,$interval,$type=NULL,$pair) {
		
         $destination = date('Y-m-d H:i:s');
		$selectFields="sum(OT.filledAmount) as volume,CO.orderTime,CO.orderDate,min(CO.Price) as low,max(CO.Price) as high,CO.price as close,(SELECT Price from `bcc_coin_order` where `pair` = ".$pair." AND `tradetime` >= '".$source."' AND `tradetime` < '".$destination."' GROUP BY `trade_id` asc LIMIT 1) as open";
		$names = array('filled', 'partially');
		$where=array('CO.pair'=>$pair,'tradetime >= '=>$source,'tradetime < '=>$destination);
		...
		$query = $this->trade_model->getleftJoinedTableData('coin_order as CO',$joins,$where,$selectFields,'','','','','','',$orderBy,$where_in);
...
//echo $this->db->last_query();die;
	...
	/*function forLowHighchart_pair($source,$interval,$type=NULL,$pair) {
        $destination = date('Y-m-d H:i:s', strtotime($source)+$interval);
		$selectFields="sum(OT.filledAmount) as volume,CO.orderTime,CO.orderDate,min(CO.Price) as low,max(CO.Price) as high,CO.price as close,(SELECT Price from `bcc_coin_order` where `pair` = ".$pair." AND `tradetime` >= '".$source."' AND `tradetime` < '".$destination."' ORDER BY `trade_id` asc LIMIT 1) as open";
		$names = array('filled', 'partially');
		$where=array('CO.pair'=>$pair,'tradetime >= '=>$source,'tradetime < '=>$destination);
		$orderBy=array('CO.trade_id','desc');
		$where_in=array('CO.status', $names);
